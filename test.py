...
# print(gaussianNumerator)
### low pass filter와 high pass filter 생성
LPF = np.exp(-gaussianNumerator*0.02 / (2*sigma*sigma))
HPF = 1 - LPF
print(LPF)
...
cv2.imshow("LPF",LPF_shift)
cv2.imshow("HPF",HPF_shift)
...
cv2.imshow("YUV Y'",y)
cv2.imshow("log",imgLog)
cv2.imshow("exp",img_exp)
cv2.imshow("out",img_out)

result=cv2.rectangle(result,(10,10),(200,200),(0,255,255),1)
c=[]
d=[]
for i in range(cols):
    for j in range(rows):
        c.append(img[j,i,2])
        d.append(result[j,i,2])
sum1=0
sum2=0


print(np.std(c),np.std(d))
...
cv2.imwrite('filtered_after.jpg', result)
